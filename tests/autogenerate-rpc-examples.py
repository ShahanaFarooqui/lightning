...
def generate_bookkeeper_examples(l2, l3, c23_2_chan_id):
    ...
    try:
        ...
        update_example(node=l3, method='bkpr-listaccountevents', params=[c23_2_chan_id], res=bkprlistaccountevents_res1, execute=False)
        ...
        update_example(node=l3, method='bkpr-listaccountevents', params={}, res=bkprlistaccountevents_res2, execute=False)
        bkprlistincome_res1 = l3.rpc.bkpr_listincome(consolidate_fees=False)
        bkprlistincome_res1 = update_list_responses(bkprlistincome_res1, list_key='income_events', slice_upto=4, update_func = lambda x, i:
            x.update(
            {
                ...
                **({'outpoint': 'txidbk' + (f"{i:02}" * 29) + ':1'} if 'outpoint' in x else {})
            }
        ), sort=True, sort_key='tag')
        update_example(node=l3, method='bkpr-listincome', params={'consolidate_fees': False}, res=bkprlistincome_res1, execute=False)
        ...
        for bkprincome in bkprlistincome_res2['income_events']:
            ...
            if deposit_income is None and bkprincome['tag'] == 'deposit':
                ...
                income_seleted = 2
            elif fee_income is None and bkprincome['tag'] == 'onchain_fee':
                fee_income = bkprincome
                income_seleted = 3
            if income_seleted is not None:
                if 'timestamp' in bkprincome:
                    REPLACE_RESPONSE_VALUES.extend([
        ...
        update_example(node=l3, method='bkpr-listincome', params={}, res=bkprlistincome_res2, execute=False)
        REPLACE_RESPONSE_VALUES.extend([
...
def generate_askrene_examples(l1, l2, l3, c12, c23_2):
    ...
    try:
        ...
        arinformchan_res1 = update_example(node=l2, method='askrene-inform-channel', params={'layer': 'test_layer', 'short_channel_id': c23_2, 'direction': 1, 'maximum_msat': 100000})
        ...
        update_example(node=l2, method='askrene-reserve', params={'path': [{'short_channel_id': c23_2, 'direction': 1, 'amount_msat': 1000000}]})
        update_example(node=l2, method='askrene-unreserve', params={'path': [{'short_channel_id': c23_2, 'direction': 1, 'amount_msat': 1000000}]})
        ...
        REPLACE_RESPONSE_VALUES.extend([
            ...
            {'data_keys': ['timestamp'], 'original_value': arinformchan_res2['constraint']['timestamp'], 'new_value': NEW_VALUES_LIST['time_at_800']},
...
def generate_utils_examples(l1, l2, l3, l4, l5, l6, c23_2, c34_2, inv_l11, inv_l22, rune_l21, bitcoind):
    ...
    try:
        ...
        update_example(node=l2, method='getlog', params={'level': 'unusual'}, res=getlog_res1, execute=False)
        ...
        update_example(node=l1, filename='sql-template', method='sql', params=[f"SELECT label, description, status FROM invoices WHERE label='label inv_l12'"], description=["A statement containing `=` needs `-o` in shell:"])
        sql_res3 = l1.rpc.sql(f"SELECT nodeid FROM nodes WHERE nodeid != x'{l3.info['id']}'")
        update_example(node=l1, filename='sql-template', method='sql', params=[f"SELECT nodeid FROM nodes WHERE nodeid != x'{NEW_VALUES_LIST['l3_id']}'"], description=['If you want to get specific nodeid values from the nodes table:'], res=sql_res3, execute=False)
        sql_res4 = l1.rpc.sql(f"SELECT nodeid FROM nodes WHERE nodeid IN (x'{l1.info['id']}', x'{l3.info['id']}')")
        update_example(node=l1, filename='sql-template', method='sql', params=[f"SELECT nodeid FROM nodes WHERE nodeid IN (x'{NEW_VALUES_LIST['l1_id']}', x'{NEW_VALUES_LIST['l3_id']}')"], description=["If you want to compare a BLOB column, `x'hex'` or `X'hex'` are needed:"], res=sql_res4, execute=False)
        update_example(node=l1, filename='sql-template', method='sql', params=['SELECT peer_id, to_us_msat, total_msat, peerchannels_status.status FROM peerchannels INNER JOIN peerchannels_status ON peerchannels_status.row = peerchannels.rowid'], description=['Related tables are usually referenced by JOIN:'])
...
def generate_backup_recovery_examples(node_factory, l4, l5, l6):
    ...
    try:
        ...
        update_example(node=l4, method='getemergencyrecoverdata', params={}, res='emergencyrecoverdata' + ('01' * 827), execute=False)
...
def generate_list_examples(l1, l2, l3, c12, c23_2, inv_l31, inv_l32, offer_l23, inv_req_l1_l22):
    """Generates lists rpc examples"""
    try:
        logger.info('Lists Start...')
        global REPLACE_RESPONSE_VALUES
        # Transactions Lists
        FUNDS_LEN = 3
        ...
        update_example(node=l1, method='listfunds', params={}, res=listfunds_res1, execute=False)
        ...
        update_example(node=l2, method='listforwards', params={'in_channel': c12, 'out_channel': c23_2, 'status': 'settled'}, res=listforwards_res1, execute=False)
        ...
        update_example(node=l2, method='listforwards', params={}, res=listforwards_res2, execute=False)

        listinvoices_res1 = l2.rpc.listinvoices(label='lbl_l21')
        listinvoices_res1 = update_list_responses(listinvoices_res1, list_key='invoices', slice_upto=5, update_func=lambda x, i: x.update({'paid_at': NEW_VALUES_LIST['time_at_850'] + (i * 10000)}))
        update_example(node=l2, method='listinvoices', params={'label': 'lbl_l21'}, res=listinvoices_res1, execute=False)
        listinvoices_res2 = l2.rpc.listinvoices()
        listinvoices_res2 = update_list_responses(listinvoices_res2, list_key='invoices', slice_upto=5, update_func=lambda x, i: x.update({'paid_at': NEW_VALUES_LIST['time_at_850'] + (i * 10000)}))
        update_example(node=l2, method='listinvoices', params={}, res=listinvoices_res2, execute=False)

        listhtlcs_res1 = l1.rpc.listhtlcs(c12)
        listhtlcs_res1 = update_list_responses(listhtlcs_res1, list_key='htlcs')
        update_example(node=l1, method='listhtlcs', params=[c12], res=listhtlcs_res1, execute=False)
        listhtlcs_res2 = l1.rpc.listhtlcs()
        listhtlcs_res2 = update_list_responses(listhtlcs_res2, list_key='htlcs')
        update_example(node=l1, method='listhtlcs', params={}, res=listhtlcs_res2, execute=False)
        ...
        update_example(node=l1, method='listsendpays', params={'bolt11': inv_l31['bolt11']}, res=listsendpays_res1, execute=False)
        ...
        update_example(node=l1, method='listsendpays', params={}, res=listsendpays_res2, execute=False)

        listpays_res1 = l2.rpc.listpays(bolt11=inv_l32['bolt11'])
        listpays_res1 = update_list_responses(listpays_res1, list_key='pays')
        update_example(node=l2, method='listpays', params={'bolt11': inv_l32['bolt11']}, res=listpays_res1, execute=False)
        listpays_res2 = l2.rpc.listpays()
        listpays_res2 = update_list_responses(listpays_res2, list_key='pays')
        update_example(node=l2, method='listpays', params={}, res=listpays_res2, execute=False)
        ...
        update_example(node=l1, method='listtransactions', params={}, res=listtransactions_res1, execute=False)
        ...
        update_example(node=l2, method='listclosedchannels', params={}, res=listclosedchannels_res1, execute=False)
        ...
        update_example(node=l2, method='listconfigs', params={}, res=listconfigs_res3, execute=False)
        ...
        update_example(node=l1, method='listpeerchannels', params={'id': l2.info['id']}, res=listpeerchannels_res1, execute=False)
        listpeerchannels_res2 = l1.rpc.listpeerchannels()
        listpeerchannels_res2 = update_list_responses(listpeerchannels_res2, list_key='channels', slice_upto=3)
        update_example(node=l1, method='listpeerchannels', params={}, res=listpeerchannels_res2, execute=False)

        listchannels_res1 = l1.rpc.listchannels(c12)
        listchannels_res1 = update_list_responses(listchannels_res1, list_key='channels', slice_upto=5, update_func=lambda x, i: x.update({'last_update': NEW_VALUES_LIST['time_at_850'] + (i * 10000)}))
        update_example(node=l1, method='listchannels', params={'short_channel_id': c12}, res=listchannels_res1, execute=False)
        listchannels_res2 = l1.rpc.listchannels()
        listchannels_res2 = update_list_responses(listchannels_res2, list_key='channels', slice_upto=5, update_func=lambda x, i: x.update({'last_update': NEW_VALUES_LIST['time_at_850'] + (i * 10000)}))
        update_example(node=l1, method='listchannels', params={}, res=listchannels_res2, execute=False)
        ...
        update_example(node=l2, method='listnodes', params={'id': l3.info['id']}, res=listnodes_res1, execute=False)
        ...
        update_example(node=l2, method='listnodes', params={}, res=listnodes_res2, execute=False)
        ...
        update_example(node=l2, method='listpeers', params={'id': l3.info['id']}, res=listpeers_res1, execute=False)
        ...
        update_example(node=l2, method='listpeers', params={}, res=listpeers_res2, execute=False)
        ...
        update_example(node=l2, method='listoffers', params={'active_only': True}, res=listoffers_res1, execute=False)
        ...
        update_example(node=l2, method='listoffers', params=[offer_l23['offer_id']], res=listoffers_res2, execute=False)
        ...
        update_example(node=l2, method='listinvoicerequests', params={}, res=listinvoicerequests_res2, execute=False)

        logger.warning('List funds:')
        logger.warning(l1.rpc.listfunds())
        logger.warning('List transactions:')
        logger.warning(l1.rpc.listtransactions())
        logger.warning('List peer channels:')
        logger.warning(l1.rpc.listpeerchannels())
        ...
        for i, transaction in enumerate(listtransactions_res1['transactions'], start=1):
            if i == 1:
                for input in transaction['inputs']:
                    if input['txid'] == next((value['original_value'] for value in REPLACE_RESPONSE_VALUES if value['new_value'] == NEW_VALUES_LIST['c12_txid']), None):
                        transaction['inputs'] = [input]
                        break
            transaction_new_values = [
                {'data_keys': ['hash'], 'original_value': transaction['hash'], 'new_value': 'txid' + (('7000' + str(i)) * 11)},
                {'data_keys': ['rawtx'], 'original_value': transaction['rawtx'], 'new_value': '02000000000101lstx' + (('7000' + str(i)) * 34)},
                {'data_keys': ['locktime'], 'original_value': transaction['locktime'], 'new_value': 549000000 + (i * 100)},
            ]
            for j, input in enumerate(transaction['inputs'], start=1):
                transaction_new_values.extend([
                    {'data_keys': ['txid'], 'original_value': input['txid'], 'new_value': 'txid' + (('600' + str(i) + str(j)) * 12)},
                    {'data_keys': ['index'], 'original_value': input['index'], 'new_value': 1},
                    {'data_keys': ['sequence'], 'original_value': input['sequence'], 'new_value': 2158500000 + (i * 1000) + (j * 100)},
                ])
            for k, output in enumerate(transaction['outputs'], start=1):
                transaction_new_values.extend([
                    {'data_keys': ['scriptPubKey'], 'original_value': output['scriptPubKey'], 'new_value': 'scriptpubkey' + ((f"{i:02}" + f"{k:02}") * 14)},
                    {'data_keys': ['amount_msat'], 'original_value': output['amount_msat'], 'new_value': 201998900000 + (i * 1000) + (k * 100)},
                ])
            REPLACE_RESPONSE_VALUES.extend(transaction_new_values)
...
def test_generate_examples(node_factory, bitcoind, executor):
    ...
    try:
        ...
        generate_utils_examples(l1, l2, l3, l4, l5, l6, c23_2, c34_2, inv_l11, inv_l22, rune_l21, bitcoind)
        ...
        generate_list_examples(l1, l2, l3, c12, c23_2, inv_l31, inv_l32, offer_l23, inv_req_l1_l22)
